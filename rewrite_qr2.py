...
import { AlertCircle, Camera, Image as ImageIcon, Loader2 } from 'lucide-react';
...
export default function QRScanner({ onScanSuccess, onScanFailure, onClose }: QRScannerProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Using qr-scanner library for much better image recognition
      ...
      if (result && result.data) {
        ...
        alert("Không tìm thấy mã QR trong ảnh. Vui lòng chụp rõ hơn hoặc cắt ảnh vừa với mã QR.");
      }
    } catch (err: any) {
      console.error(err);
      if (err === 'No QR code found') {
        alert("Không tìm thấy mã QR trong ảnh. Vui lòng chụp rõ mã hơn, đảm bảo ánh sáng tốt.");
      } else {
        alert("Có lỗi xảy ra khi phân tích ảnh: " + err);
      }
    } finally {
        ...
        <div className="p-4 bg-gray-50 flex-1 overflow-y-auto">
          ...
          <div className="mt-4 text-center">
            ...
            <button 
              ...
              className="w-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold py-3.5 px-6 rounded-xl hover:bg-emerald-100 transition-colors flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isProcessing ? (
                <>
